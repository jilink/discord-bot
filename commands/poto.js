...
module.exports = {
	...
	async execute(interaction, client) {
		const message = await interaction.reply({ content : 'Qui veut jouer ?', fetchReply: true});
		var ids = new Set();
		message.react('👍')
			.then(() => message.react('🚫'));
		...
		try{
			...
			const filter = (reaction, user) => {
				//console.log(user);
				return['👍', '🚫'].includes(reaction.emoji.name) && user.id != clientId;
			}
			const collector = message.createReactionCollector({filter,  time: 5000, min:1});
			...
			collector.on('end', (collector, reason) => {
				...
				message.edit('Début du jeu');
					message.reactions.removeAll()
						.catch(error => console.error('Erreur lors de la prise de réactions', error));
				...
				const mot1 = mots[Math.floor(Math.random()*array.length)];
				client.users.cache.get(maitre).send(`Vous êtes le maître. Le mot est : ${mot1}`);
				client.users.cache.get(traitre).send(`Vous êtes le traître. Le mot est : ${mot1}`);
